...
const CalendarPage: React.FC = () => {
  ...
  const CellContent = ({ ep }: { ep: Episode }) => {
      // Determine image style based on settings
      const imgClass = settings.calendarPosterFillMode === 'contain' 
      ...
      return (
          <div className="absolute inset-0 flex flex-col justify-end p-2 sm:p-3 overflow-hidden">
              ...
              <div className="absolute inset-0 z-0">
                   ...
                   {settings.calendarPosterFillMode === 'contain' && (
                       <div 
                  ...
                  <img 
                      ...
                      className={`w-full h-full ${imgClass} opacity-60`}
                      alt=""
                  />
                  ...
                  <div className="flex items-center gap-2">
                      {ep.is_movie ? (
                          ...
                          `}>
                              ...
                              <span className="hidden sm:inline">{ep.release_type === 'theatrical' ? 'Cinema' : 'Digital'}</span>
                          </div>
                      ) : (
    ...
    <div className={`flex flex-col h-full gap-4 ${settings.compactCalendar ? 'overflow-hidden' : ''}`}>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2 shrink-0">
        <div className="flex items-center gap-6">
            ...
            <div className="hidden md:flex items-center bg-zinc-800 rounded-lg p-0.5 border border-zinc-700">
                <button 
                    onClick={() => updateSettings({ viewMode: 'grid' })}
                    className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all flex items-center gap-2 ${isGridView ? 'bg-indigo-600 text-white shadow-sm' : 'text-zinc-400 hover:text-white'}`}
                >
        ...
        <div className="flex items-center gap-2">
           <button 
            ...
            <button onClick={goToToday} className="text-xs font-bold text-zinc-400 hover:text-white px-2 uppercase tracking-wider">
                Today
            </button>
      ...
      {loading && activeDays.length === 0 ? (
          ...
          <>
            ...
            {isGridView && (
                <div className={`
                    hidden md:flex flex-col bg-zinc-900/50 border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl
                ...
                `}>
                    ...
                    {/* Cells */}
                    <div className="grid grid-cols-7 flex-1 bg-zinc-950">
                        {calendarDays.map((day, idx) => {
                            ...
                            // Borders logic: right border for all except last col, bottom border for all except last row
                            ...
                            return (
                                ...
                                >
                                    {/* Date Label */}
                                    <div className={`
                                        absolute top-2 right-2 text-xs font-medium z-20 px-1.5 py-0.5 rounded
                                        ...
                                        dayEpisodes.length === 1 ? (
                                            ...
                                            <div className="absolute inset-0 p-1 flex flex-col gap-1 overflow-hidden z-10 pt-8">
                                                ...
                                                {dayEpisodes.length > 3 && (
                                                    <div className="mt-auto text-[9px] text-center text-zinc-500 font-medium bg-zinc-900/50 py-0.5">
                                    ...
                                    {hasEpisodes && (
                                        <div className="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-20" />
                                    )}
                                </div>
                            );
                        })}
            ...
            <div className={`
                flex-1 overflow-y-auto px-1 custom-scrollbar
                ${isGridView ? 'md:hidden' : 'flex flex-col gap-4'}
            `}>
                {activeDays.length === 0 ? (
                    ...
                    activeDays.map(day => {
                        ...
                        return (
                            <div key={day.toString()} className="flex gap-4">
                                ...
                                <div className="flex-1 space-y-2 pb-4 border-b border-zinc-800">
                                    {eps.map(ep => (
                                        <div 
                                            key={`${ep.show_id}-${ep.id}`}
                                            onClick={() => setSelectedDate(day)}
                                            className="surface-card rounded-xl p-3 flex gap-3 cursor-pointer group bg-zinc-900 border border-zinc-800"
                                        >
                                            <div className="relative w-12 h-16 shrink-0 rounded-md overflow-hidden bg-black">
