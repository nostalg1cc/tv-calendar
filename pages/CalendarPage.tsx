...
const CalendarPage: React.FC = () => {
  ...
  const CellContent = ({ ep, isMobile = false }: { ep: Episode, isMobile?: boolean }) => {
      ...
      const imgClass = isContain ? 'object-contain relative z-10 drop-shadow-xl' : 'object-cover opacity-80 group-hover:opacity-100 transition-opacity';
      ...
      return (
          <div className="absolute inset-0 flex flex-col justify-end p-2 sm:p-3 overflow-hidden">
              <div className="absolute inset-0 z-0">
                  ...
                  <img 
                      src={imageUrl} 
                      className={`w-full h-full ${imgClass} ${isWatched ? 'grayscale opacity-50' : ''}`}
                      alt=""
                      loading="lazy"
                  />
              ...
              <div className="relative z-20 w-full pointer-events-none">
                  ...
                  <div className="flex items-center gap-2">
                      {ep.is_movie ? (
                          ...
                          <div className={`flex items-center gap-1.5 px-1.5 py-0.5 rounded-md text-[10px] font-medium border border-white/10 shadow-sm backdrop-blur-md ${isWatched ? 'text-zinc-500 bg-black/40' : 'text-slate-200 bg-black/60'}`}>
  ...
  return (
    <div className="flex h-full overflow-hidden xl:gap-6">
        {/* Main Content Area */}
        <div className={`flex-1 flex flex-col min-w-0 h-full gap-2 p-4 md:p-0 ${settings.compactCalendar ? 'overflow-hidden' : ''}`}>
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className={`
                                                ...
                                                ${isSingle ? 'bg-black/40 backdrop-blur-md border border-white/10 text-white shadow-sm' : ''}
                                            `}>
                                                {format(day, 'd')}
                                            </div>
