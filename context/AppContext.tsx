...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Re-bucket episodes on setting change
  useEffect(() => {
          ...
          setInteractions(prev => {
              ...
              epsToMark.forEach(item => {
                  ...
                  if (!next[key]?.is_watched) {
                      next[key] = item;
                      manualOverridesRef.current[key] = true; 
                  }
              });
              return next;
          });
  ...
  // -- Full Sync & Migration Logic --
  const performFullSync = async (config?: Partial<AppSettings>) => { 
      if (!user?.isCloud || !supabase || !user.id) return; 
      setIsSyncing(true); 
      setLoading(true); 
      
      try { 
          // 1. Clean up local caches
          await del(DB_KEY_EPISODES);
          await del(DB_KEY_META);
          setEpisodes({});

          // 2. If config provided (from modal), update settings first
          if (config) {
              await updateSettings(config);
          }

          // 3. Clear cloud calendar cache to force regeneration
          ...
          setUser(prev => prev ? ({ ...prev, fullSyncCompleted: true }) : null);
  ...
  }, [user, allTrackedShows, watchlist, fullSyncRequired, settings.timeShift, settings.timezone, settings.autoSync, settings.ignoreSpecials, getAdjustedDate]);
  ...
  const updateSettings = async (newSettings: Partial<AppSettings>) => { 
      setSettings(prev => { 
          ...
          const localKeys = ['viewMode', 'mobileNavLayout', 'v2SidebarMode']; 
          ...
          if (user?.isCloud && supabase && user.id) {
              ...
              supabase.from('profiles').update({ settings: payload, updated_at: new Date().toISOString() }).eq('id', user.id).then(({error}) => { if(error) console.error("Failed to sync settings", error); }); 
          }
  ...
  const addToWatchlist = async (show: TVShow) => { 
      ...
      if (settings.reminderStrategy === 'ask') setReminderCandidate(show);
          ...
          await addReminder({ tmdb_id: show.id, media_type: show.media_type, show_name: show.name, scope: isMovie ? 'movie_digital' : 'all', offset_minutes: 0 });
      }
  ...
  const removeFromWatchlist = async (showId: number) => {
    ...
    } else { if (user?.isCloud && supabase) await supabase.from('watchlist').delete().match({ user_id: user.id, tmdb_id: showId }); }
    if (updateTimeoutRef.current) clearTimeout(updateTimeoutRef.current);
    updateTimeoutRef.current = setTimeout(() => { refreshEpisodes(true); }, 2000);
  };
  ...
  const unsubscribeFromList = async (listId: string) => {
      ...
      if (listToRemove) {
          ...
          if (showsToPurge.length > 0) {
              ...
              if (user?.isCloud && supabase) supabase.from('user_calendar_events').delete().in('tmdb_id', showsToPurge).eq('user_id', user.id).then();
          }
      }
  ...
  const getSyncPayload = useCallback(() => { 
      ...
      delete (settingsToExport as any).viewMode; 
      delete (settingsToExport as any).mobileNavLayout; 
