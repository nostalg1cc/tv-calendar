...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
      ...
      try {
          ...
          if (profile) { 
              ...
              if (remoteInteractions) { 
                  console.log(`Loaded ${remoteInteractions.length} interactions from cloud.`);
  ...
  const saveInteraction = async (interaction: Interaction) => { 
      ...
      try { 
          const payload = { 
              ...
              is_watched: !!interaction.is_watched, // Ensure boolean
          ...
          // Clean conflict string without spaces
          ...
          else console.log("Interaction saved successfully:", payload);
      } catch (e: any) { 
          console.error("Interaction save error", e); 
      } 
  };
  const toggleWatched = async (id: number, mediaType: 'tv' | 'movie') => { 
      ...
      // Ensure defaults of -1 are set here for local state too to match DB schema
