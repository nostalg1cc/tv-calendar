...
const V2Library: React.FC = () => {
    ...
    const getNextEpisodeDate = (showId: number): string | null => {
        ...
        const allEps = Object.values(episodes).flat() as Episode[];
    ...
    const filteredItems = useMemo(() => {
        ...
        // Note: 'ended' and 'returning' are tricky without dedicated status field in TVShow type
        // relying on simplistic check for now
        ...
        return items.sort((a, b) => {
            ...
            // Date Added is not tracked in TVShow type directly (it is in watchlist array order implicitly, but allTrackedShows mixes lists)
            // Fallback to name
            return a.name.localeCompare(b.name);
        });
    }, [allTrackedShows, searchQuery, filter, sort, episodes]);
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-xs font-bold text-zinc-500 uppercase tracking-widest">
                            {filteredItems.length} Items • {allTrackedShows.filter(i => i.media_type === 'tv').length} Series • {allTrackedShows.filter(i => i.media_type === 'movie').length} Movies
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3 bg-zinc-900/50 p-1 rounded-xl border border-white/5 w-full md:w-auto">
                        ...
                        <input 
                            ...
                            className="bg-transparent border-none outline-none text-sm text-white placeholder:text-zinc-600 w-full md:w-64 py-2"
                        />
                    ...
                    <div className="flex gap-2 overflow-x-auto hide-scrollbar pb-2 md:pb-0">
                        {[
                            ...
                            <button
                                ...
                                className={`
                                    px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap border transition-all
                                    ${filter === f.id 
                    ...
                    <div className="flex items-center gap-4 ml-auto">
                        <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-500 uppercase tracking-wider">
                            ...
                            <select 
                                ...
                                className="bg-transparent text-white outline-none cursor-pointer hover:text-indigo-400 transition-colors"
                            >
                        ...
                        <div className="flex bg-zinc-900 rounded-lg p-0.5 border border-white/5">
                            <button 
                                ...
                                className={`p-1.5 rounded-md transition-all ${layout === 'grid' ? 'bg-white/10 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
                            >
                                <LayoutGrid className="w-4 h-4" />
                            </button>
                            <button 
                                onClick={() => handleLayoutChange('list')}
                                className={`p-1.5 rounded-md transition-all ${layout === 'list' ? 'bg-white/10 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
                            >
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-12">
                {filteredItems.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full opacity-30">
                        <LayoutGrid className="w-16 h-16 text-zinc-500 mb-4 stroke-1" />
                        <p className="text-sm font-bold text-zinc-500 uppercase tracking-widest">Library Empty</p>
                    </div>
                ) : (
                    layout === 'grid' ? (
                                    ...
                                    >
                                        <div className="relative aspect-[2/3] w-full bg-zinc-900 rounded-xl overflow-hidden shadow-lg border border-white/5 transition-all duration-300 group-hover:-translate-y-2 group-hover:shadow-2xl group-hover:border-white/20">
                                            ...
                                            {item.media_type === 'tv' && nextEp && (
                                                <div className="absolute top-2 left-2 px-1.5 py-0.5 bg-indigo-600 text-white text-[8px] font-black uppercase tracking-widest rounded shadow-lg">
                                                    New
                                                </div>
                                            )}
                                            ...
                                            <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                                                <button 
                                                    ...
                                                    className="w-8 h-8 rounded-full bg-black/60 backdrop-blur-md flex items-center justify-center text-white hover:bg-red-600 transition-colors"
                                                >
                                            ...
                                            <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black via-black/80 to-transparent pt-10">
                                                <h4 className={`text-xs font-bold leading-tight line-clamp-2 ${isWatched ? 'text-zinc-500 line-through' : 'text-white'}`}>
                                                    {item.name}
                                                </h4>
                                                <div className="flex items-center justify-between mt-1">
                                                    <span className={`text-[9px] font-bold uppercase tracking-wider ${nextEp ? 'text-emerald-400' : 'text-zinc-500'}`}>
                                                        {nextEp ? format(parseISO(nextEp), 'MMM d') : (item.media_type === 'movie' ? item.first_air_date?.split('-')[0] : 'Ended')}
                                                    </span>
                                                    {item.vote_average > 0 && <span className="text-[8px] font-bold text-yellow-500 flex items-center gap-0.5"><Star className="w-2 h-2 fill-current" /> {item.vote_average.toFixed(1)}</span>}
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {filteredItems.map(item => {
                                ...
                                return (
                                    <div 
                                        key={item.id}
                                        onClick={() => setSelectedItem(item)}
                                        className="flex items-center gap-4 p-3 rounded-xl bg-zinc-900/40 border border-white/5 hover:bg-white/[0.03] transition-colors cursor-pointer group"
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-2">
                                                <h4 className={`text-sm font-bold text-white truncate ${isWatched ? 'line-through text-zinc-500' : ''}`}>{item.name}</h4>
                                                {item.media_type === 'movie' && <span className="text-[8px] px-1.5 py-0.5 rounded border border-white/10 text-zinc-400 font-black uppercase tracking-wider">Movie</span>}
                                            </div>
                                            <div className="flex items-center gap-4 mt-1 text-xs text-zinc-500">
                                                <span className={`font-medium ${nextEp ? 'text-emerald-400' : ''}`}>
                                                    {nextEp ? `Next: ${format(parseISO(nextEp), 'MMM d')}` : (item.media_type === 'movie' ? `Released: ${item.first_air_date}` : 'Status: Ended')}
                                                </span>
                                                <span className="hidden sm:inline">•</span>
                                                <span className="hidden sm:inline">{item.vote_average.toFixed(1)} Rating</span>
                                            </div>
                                        </div>
                ...
                <div className="fixed inset-0 z-[150] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in" onClick={() => setDeleteId(null)}>
                    ...
                    >
                        ...
                        <div className="flex justify-end gap-3">
                            <button onClick={() => setDeleteId(null)} className="px-4 py-2 rounded-lg text-sm font-bold text-zinc-400 hover:text-white">Cancel</button>
                            <button 
                                onClick={() => { if(deleteId) removeFromWatchlist(deleteId); setDeleteId(null); }} 
                                className="px-4 py-2 rounded-lg text-sm font-bold bg-red-600 hover:bg-red-500 text-white shadow-lg"
                            >
                                Remove
                            </button>
                        </div>
